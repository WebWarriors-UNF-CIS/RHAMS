import { Entity, FieldType, Fields, Relations } from "remult";
...
@FieldType<Location>({
  displayValue: (_, value) =>
    value ? value.city + ", " + value.state + ", " + value.county : "",
})
...
class Persion {
  @Relations.toOne(() => Location, {
    displayValue: (p: Persion) =>
      p.location.city + ", " + p.location.state + ", " + p.location.county,
  })
  location!: Location;
}
// personRepo = repo(Person)
// <span> {personRepo.fields.location.displayValue(person)}</span>
*/
